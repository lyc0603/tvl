...
from scripts.process.risk_slashing import result_dict
...
def risk_analysis_data_gen(
    risk_type: Literal["slashing", "price"] = "price",
    num_node_operator: Optional[int] = None,
    risk_name: str = "risk_dict.json",
    depeg_name: str = "depeg_dict.json",
) -> None:
    """
    Function to generate the sensitivity test data
    """
    ...
    for params, params_grid in params_dict.items():
        ...
        for params_idx, target_params in enumerate(params_grid[params]):
            ...
            for event, _ in SAMPLE_DATA_DICT.items():
                ...
                for x in tqdm(range(0, 101, 1)):

                    eth_decline_pct = (
                        x
                        if risk_type == "price"
                        else -result_dict[event][num_node_operator]["loss"][x] * 100
                    )
                    ...
                    for token, amount in plain_token_dict[event]["eth"].items():
                        if token in PLAIN_TOKEN_LIST:
                            non_plf_tvr_drop += (
                                amount * eth_decline_pct / 100 * eth_price
                            )
                    ...
                    risk_dict[params][target_params][event]["eth_decline_pct"].append(x)
    ...
    with open(
        PROCESSED_DATA_PATH / "risk" / risk_name,
        "w",
        encoding="utf-8",
    ) as f:
    ...
    with open(
        PROCESSED_DATA_PATH / "risk" / depeg_name,
        "w",
        encoding="utf-8",
    ) as f:
...
if __name__ == "__main__":
    # risk_analysis_data_gen()

    for num_node_operator in range(1, 4, 1):
        risk_analysis_data_gen(
            "slashing",
            num_node_operator,
            f"risk_dict_slashing_{num_node_operator}.json",
            f"depeg_dict_slashing_{num_node_operator}.json",
        )
